...
class Album extends EntityRepository
{
    ...
    protected function getViewQuickNavParameters($context = '', $args = [])
    {
        ...
        $parameters = [];
        $parameters['catIdList'] = ModUtil::apiFunc('MUMUImageModule', 'category', 'retrieveCategoriesFromRequest', ['ot' => 'album', 'source' => 'GET']);
    ...
    public function truncateTable()
    {
        ...
        $qb->delete('MU\MUImageModule\Entity\AlbumEntity', 'tbl');
        $query = $qb->getQuery();
        ...
        $logArgs = ['app' => 'MUMUImageModule', 'user' => $serviceManager->get('zikula_users_module.current_user')->get('uname'), 'entity' => 'album'];
    ...
    public function updateCreator($userId, $newUserId)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('MU\MUImageModule\Entity\AlbumEntity', 'tbl')
        ...
        $logger = $serviceManager->get('logger');
        $logArgs = ['app' => 'MUMUImageModule', 'user' => $serviceManager->get('zikula_users_module.current_user')->get('uname'), 'entities' => 'albums', 'userid' => $userId];
    ...
    public function selectWherePaginated($where = '', $orderBy = '', $currentPage = 1, $resultsPerPage = 25, $useJoins = true, $slimMode = false)
    {
        ...
        if (!$hasFilters) {
            ...
            if ($page > 1 || isset($_GET['pos'])) {
                ...
                $session->set('MUMUImageModuleAlbumsCurrentPage', $page);
            } else {
                // restore current page from session
                $page = $session->get('MUMUImageModuleAlbumsCurrentPage', 1);
    ...
    public function addCommonViewFilters(QueryBuilder $qb)
    {
        ...
        foreach ($parameters as $k => $v) {
            if ($k == 'catId') {
                 ...
                 */
                $qb = ModUtil::apiFunc('MUMUImageModule', 'category', 'buildFilterClauses', ['qb' => $qb, 'ot' => 'album', 'catids' => $v]);
    ...
    protected function getCountQuery($where = '', $useJoins = true)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select($selection)
           ->from('MU\MUImageModule\Entity\AlbumEntity', 'tbl');
    ...
    protected function genericBaseQueryAddWhere(QueryBuilder $qb, $where = '')
    {
        ...
        $categoryProperties = ModUtil::apiFunc('MUMUImageModule', 'category', 'getAllProperties', ['ot' => 'album']);
        foreach ($categoryProperties as $propertyName => $registryId) {
            $config['plugins'][] = new CategoryFilter('MUMUImageModule', $propertyName, 'categories' . ucfirst($propertyName));
        }

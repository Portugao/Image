...
class Picture extends EntityRepository
{
    ...
    public function getAdditionalTemplateParameters($context = '', $args = [])
    {
        ...
        if ($context == 'controllerAction') {
            ...
            $imageHelper = $serviceManager->get('mu_muimage_module.image_helper');
            ...
            if (in_array($args['action'], ['display', 'view'])) {
                ...
                $templateParameters['relationThumbPreset'] = $imageHelper->getCustomPreset('', '', 'MUMUImageModule_relateditem', $context, $args);
            }
        }
    ...
    protected function getViewQuickNavParameters($context = '', $args = [])
    {
        ...
        $parameters['showDescription'] = $this->request->query->get('showDescription', '');
    ...
    public function truncateTable()
    {
        ...
        $qb->delete('MU\MUImageModule\Entity\PictureEntity', 'tbl');
        $query = $qb->getQuery();
        ...
        $logArgs = ['app' => 'MUMUImageModule', 'user' => $serviceManager->get('zikula_users_module.current_user')->get('uname'), 'entity' => 'picture'];
    ...
    public function updateCreator($userId, $newUserId)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('MU\MUImageModule\Entity\PictureEntity', 'tbl')
        ...
        $logger = $serviceManager->get('logger');
        $logArgs = ['app' => 'MUMUImageModule', 'user' => $serviceManager->get('zikula_users_module.current_user')->get('uname'), 'entities' => 'pictures', 'userid' => $userId];
    ...
    public function selectWherePaginated($where = '', $orderBy = '', $currentPage = 1, $resultsPerPage = 25, $useJoins = true, $slimMode = false)
    {
        ...
        if (!$hasFilters) {
            ...
            if ($page > 1 || isset($_GET['pos'])) {
                ...
                $session->set('MUMUImageModulePicturesCurrentPage', $page);
            } else {
                // restore current page from session
                $page = $session->get('MUMUImageModulePicturesCurrentPage', 1);
    ...
    public function addCommonViewFilters(QueryBuilder $qb)
    {
        ...
        foreach ($parameters as $k => $v) {
            if ($k == 'catId') {
                 ...
                 */
                $qb = ModUtil::apiFunc('MUMUImageModule', 'category', 'buildFilterClauses', ['qb' => $qb, 'ot' => 'picture', 'catids' => $v]);
            } elseif (in_array($k, ['q', 'searchterm'])) {
                // quick search
                if (!empty($v)) {
                    $qb = $this->addSearchFilter($qb, $v);
                }
            } elseif (in_array($k, ['showTitle', 'showDescription', 'albumImage'])) {
                // boolean filter
                if ($v == 'no') {
                    $qb->andWhere('tbl.' . $k . ' = 0');
                } elseif ($v == 'yes' || $v == '1') {
    ...
    protected function applyDefaultFilters(QueryBuilder $qb, $parameters = [])
    {
        ...
        if ($currentLegacyControllerType == 'admin' && $currentModule == 'MUMUImageModule') {
            return $qb;
        }
    ...
    protected function getCountQuery($where = '', $useJoins = true)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select($selection)
           ->from('MU\MUImageModule\Entity\PictureEntity', 'tbl');
    ...
    protected function genericBaseQueryAddWhere(QueryBuilder $qb, $where = '')
    {
        ...
        $showOnlyOwnEntries = /*$request->query->getDigits('own', */$varHelper->get('MUMUImageModule', 'showOnlyOwnEntries', 0)/*)*/;

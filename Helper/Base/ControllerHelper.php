...
class ControllerHelper
{
    ...
    /**
     * Returns an array of all allowed object types in MUMUImageModule.
     *
    ...
    /**
     * Returns the default object type in MUMUImageModule.
     *
     * @param string $context Usage context (allowed values: controllerAction, api, helper, actionHandler, block, contentType, util)
     * @param array  $args    Additional arguments
     *
    ...
    public function getFileBaseFolder($objectType, $fieldName, $ignoreCreate = false)
    {
        ...
        $basePath = $this->container->getParameter('datadir') . '/MUMUImageModule/';
    ...
    protected function checkAndCreateUploadFolder($objectType, $fieldName, $allowedExtensions = '')
    {
        ...
        if (!$fs->exists($uploadPath)) {
            try {
                ...
                $this->logger->error('{app}: The upload directory {directory} does not exist and could not be created.', ['app' => 'MUMUImageModule', 'directory' => $uploadPath]);
        ...
        if (!is_writable($uploadPath)) {
            try {
                ...
                $this->logger->error('{app}: The upload directory {directory} exists but is not writable by the webserver.', ['app' => 'MUMUImageModule', 'directory' => $uploadPath]);
        ...
        $htaccessFileTemplate = 'modules/MUMUImageModule/Resources/docs/htaccessTemplate';
        if (!$fs->exists($htaccessFilePath) && $fs->exists($htaccessFileTemplate)) {
            try {
                ...
                $this->logger->error('{app}: An error occured during creation of the .htaccess file in directory {directory}.', ['app' => 'MUMUImageModule', 'directory' => $uploadPath]);
            }
        }

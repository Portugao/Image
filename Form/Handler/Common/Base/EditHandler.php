...
namespace MU\MUImageModule\Form\Handler\Common\Base;
...
use System;
use UserUtil;
use MU\MUImageModule\UploadHandler;
...
class EditHandler
{
    ...
    public function processForm(array $templateParameters)
    {
        ...
        $this->permissionComponent = 'MUMUImageModule:' . $this->objectTypeCapital . ':';
    
        $this->idFields = ModUtil::apiFunc('MUMUImageModule', 'selection', 'getIdFields', ['ot' => $this->objectType]);
        ...
        if ($this->templateParameters['mode'] == 'edit') {
            ...
            if (true === $this->hasPageLockSupport && ModUtil::available('ZikulaPageLockModule')) {
                ...
                $lockName = 'MUMUImageModule' . $this->objectTypeCapital . $this->createCompositeIdentifier();
        ...
        if (false === $actions || !is_array($actions)) {
            ...
            $logArgs = ['app' => 'MUMUImageModule', 'user' => $this->container->get('zikula_users_module.current_user')->get('uname'), 'entity' => $this->objectType, 'id' => $entity->createCompositeIdentifier()];
    ...
    {
        $entity = ModUtil::apiFunc('MUMUImageModule', 'selection', 'getEntity', ['ot' => $this->objectType, 'id' => $this->idValues]);
        if (null === $entity) {
    ...
    protected function initEntityForCreation()
    {
        ...
        if (!empty($templateId)) {
            ...
            if (true === $this->hasTemplateId) {
                ...
                $entityT = ModUtil::apiFunc('MUMUImageModule', 'selection', 'getEntity', ['ot' => $this->objectType, 'id' => $templateIdValues]);
                if (null === $entityT) {
                    throw new NotFoundHttpException($this->__('No such item.'));
                }
        ...
        if (is_null($entity)) {
            $factory = $this->container->get('mu_muimage_module.' . $this->objectType . '_factory');
    ...
    public function handleCommand(&$args)
    {
        ...
        if ($entity->supportsHookSubscribers() && !in_array($action, ['reset', 'cancel'])) {
            $hookHelper = $this->container->get('mu_muimage_module.hook_helper');
        ...
        if ($isRegularAction || $action == 'delete') {
            ...
            if ($entity->supportsHookSubscribers()) {
                ...
                if ($action != 'delete') {
                    ...
                    $url = new RouteUrl('mumuimagemodule_' . $this->objectType . '_display', $urlArgs);
                }
    ...
    protected function addDefaultMessage($args, $success = false)
    {
        ...
        $logger = $this->container->get('logger');
        $logArgs = ['app' => 'MUMUImageModule', 'user' => $this->container->get('zikula_users_module.current_user')->get('uname'), 'entity' => $this->objectType, 'id' => $this->entityRef->createCompositeIdentifier()];
        if (true === $success) {
    ...
    public function fetchInputData(&$args)
    {
        ...
        if (isset($formData['additionalNotificationRemarks']) && $formData['additionalNotificationRemarks'] != '') {
            $this->request->getSession()->set('MUMUImageModuleAdditionalNotificationRemarks', $formData['additionalNotificationRemarks']);
        }

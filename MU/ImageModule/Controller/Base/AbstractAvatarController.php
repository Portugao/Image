...
/**
 * Avatar controller base class.
 */
abstract class AbstractAvatarController extends AbstractController
{
    /**
    ...
    {
        $avatar = $this->get('mu_image_module.entity_factory')->getRepository('avatar')->selectById($id);
        if (null === $avatar) {
            throw new NotFoundHttpException($this->__('No such avatar found.'));
        }
    
        return $this->displayInternal($request, $avatar, true);
    }
    ...
    public function displayAction(Request $request, $id)
    {
        ...
        return $this->displayInternal($request, $avatar, false);
    }
    ...
    protected function displayInternal(Request $request, AvatarEntity $avatar, $isAdmin = false)
    {
        ...
        $controllerHelper = $this->get('mu_image_module.controller_helper');
        $templateParameters = $controllerHelper->processDisplayActionParameters($objectType, $templateParameters, $avatar->supportsHookSubscribers());
    ...
    protected function deleteInternal(Request $request, $id, $isAdmin = false)
    {
        ...
        if (!$deleteAllowed) {
            $this->addFlash('error', $this->__('Error! It is not allowed to delete this avatar.'));
            $logger->error('{app}: User {user} tried to delete the {entity} with id {id}, but this action was not allowed.', $logArgs);
        
            return $this->redirectToRoute($redirectRoute);
        }
        
        $form = $this->createForm(DeletionType::class, $avatar);
    ...
    protected function handleSelectedEntriesActionInternal(Request $request, $isAdmin = false)
    {
        ...
        foreach ($items as $itemId) {
            ...
            if ($action == 'delete') {
                $this->addFlash('status', $this->__('Done! Item deleted.'));
                $logger->notice('{app}: User {user} deleted the {entity} with id {id}.', ['app' => 'MUImageModule', 'user' => $userName, 'entity' => 'avatar', 'id' => $itemId]);
            } else {
                $this->addFlash('status', $this->__('Done! Item updated.'));
                $logger->notice('{app}: User {user} executed the {action} workflow action for the {entity} with id {id}.', ['app' => 'MUImageModule', 'user' => $userName, 'action' => $action, 'entity' => 'avatar', 'id' => $itemId]);
            }
        }

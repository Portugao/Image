 ...
 * It aims on the avatar object type.
 */
abstract class AbstractEditHandler extends EditHandler
{
    ...
    {
        $this->objectType = 'avatar';
        $this->objectTypeCapital = 'Avatar';
        $this->objectTypeLower = 'avatar';
        ...
        if ('create' == $this->templateParameters['mode']) {
            if (!$this->modelHelper->canBeCreated($this->objectType)) {
                $this->requestStack->getCurrentRequest()->getSession()->getFlashBag()->add('error', $this->__('Sorry, but you can not create the avatar yet as other items are required which must be created before!'));
    ...
    {
        return $this->formFactory->create(AvatarType::class, $this->entityRef, $this->getFormOptions());
    }
    ...
    protected function getDefaultReturnUrl(array $args = [])
    {
        ...
        if ($objectIsPersisted) {
            // redirect to the detail page of treated avatar
    ...
    protected function getDefaultMessage(array $args = [], $success = false)
    {
        ...
        switch ($args['commandName']) {
            case 'submit':
                if ('create' == $this->templateParameters['mode']) {
                    $message = $this->__('Done! Avatar created.');
                } else {
                    $message = $this->__('Done! Avatar updated.');
                }
                break;
            case 'delete':
                $message = $this->__('Done! Avatar deleted.');
                break;
            default:
                $message = $this->__('Done! Avatar updated.');
                break;

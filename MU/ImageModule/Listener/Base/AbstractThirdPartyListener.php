...
abstract class AbstractThirdPartyListener implements EventSubscriberInterface
{
    ...
    public function getEditorHelpers(EditorHelperEvent $event)
    {
        ...
        $targetDir = 'web/modules/muimage';
        $finder = new Finder();
        if (!$this->filesystem->exists($targetDir)) {
            $this->filesystem->mkdir($targetDir, 0777);
            if (is_dir($originDir = 'modules/MU/ImageModule/Resources/public')) {
                $this->filesystem->mirror($originDir, $targetDir, Finder::create()->in($originDir));
            }
            if (is_dir($originDir = 'modules/MU/ImageModule/Resources/scribite')) {
                $targetDir .= '/scribite';
                $this->filesystem->mkdir($targetDir, 0777);
                $this->filesystem->mirror($originDir, $targetDir, Finder::create()->in($originDir));
            }
        ...
        $event->getHelperCollection()->add(
            [
                ...
                'type' => 'javascript',
                'path' => $this->requestStack->getCurrentRequest()->getBasePath() . '/web/modules/muimage/js/MUImageModule.Finder.js'
            ]
        );
    }
    ...
    public function getCKEditorPlugins(GenericEvent $event)
    {
        $event->getSubject()->add([
            'name' => 'muimagemodule',
            'path' => $this->requestStack->getCurrentRequest()->getBasePath() . '/web/modules/muimage/scribite/CKEditor/muimagemodule/',
            'file' => 'plugin.js',
    ...
    public function getQuillPlugins(GenericEvent $event)
    {
        $event->getSubject()->add([
            'name' => 'muimagemodule',
            'path' => $this->requestStack->getCurrentRequest()->getBasePath() . '/web/modules/muimage/scribite/Quill/muimagemodule/plugin.js'
        ]);
    }
    ...
    public function getSummernotePlugins(GenericEvent $event)
    {
        $event->getSubject()->add([
            'name' => 'muimagemodule',
            'path' => $this->requestStack->getCurrentRequest()->getBasePath() . '/web/modules/muimage/scribite/Summernote/muimagemodule/plugin.js'
        ]);
    }
    ...
    public function getTinyMcePlugins(GenericEvent $event)
    {
        $event->getSubject()->add([
            'name' => 'muimagemodule',
            'path' => $this->requestStack->getCurrentRequest()->getBasePath() . '/web/modules/muimage/scribite/TinyMce/muimagemodule/plugin.js'
        ]);
    }
}

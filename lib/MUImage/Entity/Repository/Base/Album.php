...
class MUImage_Entity_Repository_Base_Album extends EntityRepository
{
    ...
    protected function getViewQuickNavParameters($context = '', $args = array())
    {
        ...
        $parameters = array();
        $parameters['catIdList'] = ModUtil::apiFunc('MUImage', 'category', 'retrieveCategoriesFromRequest', array('ot' => 'album', 'source' => 'GET'));
        ...
        $parameters['albumAccess'] = isset($this->controllerArguments['albumAccess']) ? $this->controllerArguments['albumAccess'] : FormUtil::getPassedValue('albumAccess', '', 'GET');
        ...
        $parameters['notInFrontend'] = isset($this->controllerArguments['notInFrontend']) ? $this->controllerArguments['notInFrontend'] : FormUtil::getPassedValue('notInFrontend', '', 'GET');
    ...
    public function truncateTable()
    {
        ...
        $qb->delete('MUImage_Entity_Album', 'tbl');
        $query = $qb->getQuery();
    ...
    public function updateCreator($userId, $newUserId)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('MUImage_Entity_Album', 'tbl')
    ...
    public function selectWherePaginated($where = '', $orderBy = '', $currentPage = 1, $resultsPerPage = 25, $useJoins = true, $slimMode = false)
    {
        ...
        if (!$hasFilters) {
            if ($page > 1 || isset($_GET['pos'])) {
                ...
                SessionUtil::setVar('MUImageAlbumsCurrentPage', $page);
            } else {
                // restore current page from session
                $page = SessionUtil::getVar('MUImageAlbumsCurrentPage', 1);
    ...
    public function addCommonViewFilters(QueryBuilder $qb)
    {
        ...
        foreach ($parameters as $k => $v) {
            if ($k == 'catId') {
                 ...
                 */
                $qb = ModUtil::apiFunc('MUImage', 'category', 'buildFilterClauses', array('qb' => $qb, 'ot' => 'album', 'catids' => $v));
            } elseif ($k == 'searchterm') {
                // quick search
                if (!empty($v)) {
                    $qb = $this->addSearchFilter($qb, $v);
                }
            } elseif (in_array($k, array('notInFrontend'))) {
                // boolean filter
                if ($v == 'no') {
                    $qb->andWhere('tbl.' . $k . ' = 0');
                } elseif ($v == 'yes' || $v == '1') {

 ...
 */
class MUImage_Entity_Repository_Base_Picture extends EntityRepository
{
    /**
    ...
    public function getAdditionalTemplateParameters($context = '', $args = array())
    {
        ...
        if ($context == 'controllerAction') {
            ...
            if (in_array($args['action'], array('main', 'view'))) {
                ...
                $templateParameters['workflowStateItems'] = $listHelper->getEntries('picture', 'workflowState');
                $booleanSelectorItems = array(
                    array('value' => 'no', 'text' => __('No')),
                    array('value' => 'yes', 'text' => __('Yes'))
                );
                $templateParameters['showTitleItems'] = $booleanSelectorItems;
                $templateParameters['showDescriptionItems'] = $booleanSelectorItems;
                $templateParameters['albumImageItems'] = $booleanSelectorItems;
            }
    
            // initialise Imagine preset instances
            $imageHelper = new MUImage_Util_Image($serviceManager);
    
            $objectType = 'picture';
            $templateParameters[$objectType . 'ThumbPresetImageUpload'] = $imageHelper->getPreset($objectType, 'imageUpload', $context, $args);
            if (in_array($args['action'], array('display', 'view'))) {
                // use separate preset for images in related items
                $templateParameters['relationThumbPreset'] = $imageHelper->getCustomPreset('', '', 'MUImage_relateditem', $context, $args);
            }
        }
    ...
    protected function getViewQuickNavParameters($context = '', $args = array())
    {
        ...
        $parameters['showTitle'] = isset($this->controllerArguments['showTitle']) ? $this->controllerArguments['showTitle'] : FormUtil::getPassedValue('showTitle', '', 'GET');
        $parameters['showDescription'] = isset($this->controllerArguments['showDescription']) ? $this->controllerArguments['showDescription'] : FormUtil::getPassedValue('showDescription', '', 'GET');
        $parameters['albumImage'] = isset($this->controllerArguments['albumImage']) ? $this->controllerArguments['albumImage'] : FormUtil::getPassedValue('albumImage', '', 'GET');
    ...
    public function truncateTable()
    {
        ...
        $qb->delete('MUImage_Entity_Picture', 'tbl');
        $query = $qb->getQuery();
    ...
    public function updateCreator($userId, $newUserId)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('MUImage_Entity_Picture', 'tbl')
     ...
     *
     * @return array|MUImage_Entity_Picture retrieved data array or MUImage_Entity_Picture instance
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false).
     *
     * @return ArrayCollection collection containing retrieved MUImage_Entity_Picture instances
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
    ...
    public function selectWherePaginated($where = '', $orderBy = '', $currentPage = 1, $resultsPerPage = 25, $useJoins = true, $slimMode = false)
    {
        ...
        if (!$hasFilters) {
            if ($page > 1 || isset($_GET['pos'])) {
                ...
                SessionUtil::setVar('MUImagePicturesCurrentPage', $page);
            } else {
                // restore current page from session
                $page = SessionUtil::getVar('MUImagePicturesCurrentPage', 1);
    ...
    public function addCommonViewFilters(QueryBuilder $qb)
    {
        ...
        foreach ($parameters as $k => $v) {
            if ($k == 'catId') {
                 ...
                 */
                $qb = ModUtil::apiFunc('MUImage', 'category', 'buildFilterClauses', array('qb' => $qb, 'ot' => 'picture', 'catids' => $v));
            } elseif ($k == 'searchterm') {
                // quick search
                if (!empty($v)) {
                    $qb = $this->addSearchFilter($qb, $v);
                }
            } elseif (in_array($k, array('showTitle', 'showDescription', 'albumImage'))) {
                // boolean filter
                if ($v == 'no') {
                    $qb->andWhere('tbl.' . $k . ' = 0');
                } elseif ($v == 'yes' || $v == '1') {
    ...
    protected function applyDefaultFilters(QueryBuilder $qb, $parameters = array())
    {
        ...
        if ($currentLegacyControllerType == 'admin' && $currentModule == 'MUImage') {
            return $qb;
        }
    ...
    protected function addSearchFilter(QueryBuilder $qb, $fragment = '')
    {
        ...
        if (!$fragmentIsNumeric) {
            ...
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.imageUpload = \'' . $fragment . '\'';
        } else {
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.title LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
    ...
    protected function getCountQuery($where = '', $useJoins = true)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select($selection)
           ->from('MUImage_Entity_Picture', 'tbl');
    ...
    protected function genericBaseQueryAddWhere(QueryBuilder $qb, $where = '')
    {
        ...
        $showOnlyOwnEntries = (int) FormUtil::getPassedValue('own', ModUtil::getVar('MUImage', 'showOnlyOwnEntries', 0), 'GETPOST');

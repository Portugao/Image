{* purpose of this template: pictures view view in admin area *}
...
<div class="z-admin-content-pagetitle">
    ...
    {checkpermissionblock component='MUImage::' instance='.*' level="ACCESS_ADD"}
        ...
        <a href="{modurl modname='MUImage' type='admin' func='edit' ot='picture'}" title="{$createTitle}" class="z-icon-es-add">
            {$createTitle}
        </a>
    ...
    {if isset($showAllEntries) && $showAllEntries eq 1}
        ...
        <a href="{modurl modname='MUImage' type='admin' func='view' ot='picture'}" title="{$linkTitle}" class="z-icon-es-view">
            {$linkTitle}
        </a>
        {assign var='all' value=1}
    {else}
        {gt text='Show all entries' assign='linkTitle'}
        <a href="{modurl modname='MUImage' type='admin' func='view' ot='picture' all=1}" title="{$linkTitle}" class="z-icon-es-view">
            {$linkTitle}
        </a>
    {/if}
...
<table class="z-datatable">
    ...
    <tr>
        <th id="htitle" scope="col" class="z-left">
            {sortlink __linktext='Title' sort='title' currentsort=$sort sortdir=$sdir all=$all modname='MUImage' type='admin' func='view' ot='picture'}
        </th>
        <th id="hdescription" scope="col" class="z-left">
            {sortlink __linktext='Description' sort='description' currentsort=$sort sortdir=$sdir all=$all modname='MUImage' type='admin' func='view' ot='picture'}
        </th>
        <th id="hshowtitle" scope="col" class="z-center">
            {sortlink __linktext='Show title' sort='showTitle' currentsort=$sort sortdir=$sdir all=$all modname='MUImage' type='admin' func='view' ot='picture'}
        </th>
        ...
        <th id="halbum" scope="col" class="z-left">
            {sortlink __linktext='Album' sort='album' currentsort=$sort sortdir=$sdir all=$all modname='MUImage' type='admin' func='view' ot='picture'}
        </th>
    ...
    <tr class="{cycle values='z-odd, z-even'}">
        ...
        <td headers="halbum" class="z-left">
            {if isset($picture.Album) && $picture.Album ne null}
                <a href="{modurl modname='MUImage' type='admin' func='display' ot='album' id=$picture.Album.id}">
                    {$picture.Album.title|default:""}
                </a>
                <a id="albumItem{$picture.id}_rel_{$picture.Album.id}Display" href="{modurl modname='MUImage' type='admin' func='display' ot='album' id=$picture.Album.id theme='Printer'}" title="{gt text='Open quick view window'}" style="display: none">

...
{if $numItems}
{capture assign='uniqueID'}tag:{$baseurl|replace:'http://':''|replace:'/':''},{$items[0].createdDate|dateformat|default:$smarty.now|dateformat:'%Y-%m-%d'}:{modurl modname='MUImage' type=$lct func='display' ot='album'  id=$items[0].id}{/capture}
    <id>{$uniqueID}</id>
...
{foreach item='album' from=$items}
    <entry>
        ...
        <link rel="alternate" type="text/html" href="{modurl modname='MUImage' type=$lct func='display' ot='album'  id=$album.id fqurl=true}" />
        {capture assign='uniqueID'}tag:{$baseurl|replace:'http://':''|replace:'/':''},{$album.createdDate|dateformat|default:$smarty.now|dateformat:'%Y-%m-%d'}:{modurl modname='MUImage' type=$lct func='display' ot='album'  id=$album.id}{/capture}
        <id>{$uniqueID}</id>

...
{if $properties ne null && is_array($properties)}
    ...
    {foreach key='propertyName' item='propertyId' from=$properties}
        <p>
            {modapifunc modname='MUImage' type='category' func='hasMultipleSelection' ot='album' registry=$propertyName assign='hasMultiSelection'}
            ...
            &nbsp;
            {selector_category name="`$baseID`_`$categorySelectorName``$propertyName`" field='id' selectedValue=$catIds.$propertyName categoryRegistryModule='MUImage' categoryRegistryTable=$objectType categoryRegistryProperty=$propertyName defaultText=$lblDefault editLink=false multipleSize=$categorySelectorSize}
            <br{$break} />
        </p>
    {/foreach}
    {/nocache}
...
<p>
    ...
    <input type="button" id="mUImageSearchGo" name="gosearch" value="{gt text='Filter'}" />
    <br{$break} />
</p>
<br />
<br />
...
/* <![CDATA[ */
    document.observe('dom:loaded', function() {
        muimage.itemSelector.onLoad('{{$baseID}}', {{$selectedId|default:0}});
    });
/* ]]> */
</script>

{* purpose of this template: pictures rss feed *}
...
{foreach item='picture' from=$items}
    <item>
        <title><![CDATA[{if isset($picture.updatedDate) && $picture.updatedDate ne null}{$picture.updatedDate|dateformat} - {/if}{$picture->getTitleFromDisplayPattern()|notifyfilters:'muimage.filterhook.pictures'}]]></title>
        <link>{modurl modname='MUImage' type=$lct func='display' ot='picture'  id=$picture.id fqurl=true}</link>
        <guid>{modurl modname='MUImage' type=$lct func='display' ot='picture'  id=$picture.id fqurl=true}</guid>
        {if isset($picture.createdUserId)}
            ...
            <author>{usergetvar name='email' uid=$picture.createdUserId} ({$cr_name|default:$cr_uname})</author>
        {/if}

        <description>
            <![CDATA[
            {$picture.description|replace:'<br>':'<br />'}
            ]]>
        </description>
        {if isset($picture.createdDate) && $picture.createdDate ne null}
            <pubDate>{$picture.createdDate|dateformat:"%a, %d %b %Y %T +0100"}</pubDate>
        {/if}
    </item>
{/foreach}
    </channel>
